...
class Model_print_archive extends CI_Model {
...
///new start
   /* function find_tbl($dt) {
    ...
    function most_read_count_print($dt) {
    ...
    function LeadNewsPrintversion($dt) {
        // $this->db->where('pdt', $dt);
        // $tbl = $this->db->get('wsxq_p_dt');
        // print_r($tbl->result());
        // exit();
        $tbl = $this->find_tbl($dt);
...
//            redirect('my404');          ////////////////// For db error///////////////
            return NULL;
        }
    }

    function Printversion_All_1stPageNews($n_id, $dt) {
        $tbl = $this->find_tbl($dt);
    ...
    function magazineExist($dt) {
        $tbl = $this->find_tbl($dt);


        $this->db->where('n_category', '73');
        $this->db->where('n_date', $dt);
        $this->db->where('n_status', 3);
        $this->db->where('article_type', 2);
        $this->db->limit(1);
...
//print $this->db->last_query();
    ...
    function getCategory2ndNews($m_id, $n_id, $dt) {
        $tbl = $this->find_tbl($dt);
...
///End Meta Query
    ...
    function PrintHomeCatNews($m_id, $dt) {
        $tbl = $this->find_tbl($dt);
        //$dt = $this->pubdate_list();
        $this->db->where('n_category', $m_id);
        $this->db->where('n_status', '3');
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($row1['main_image'] != '') {
                ...
                echo '<img width="358" height="233" border="0" alt="' . strip_tags($row1['n_head']) . '" src="./assets/news_images/'.str_replace('-','/',$row1['n_date']).'/' . $row1['main_image'] . '" />';
                echo '</div>';
            }
...
//print $this->db->last_query();
    ...
    function getMorenewsbyID($n_category, $n_id, $dt) {
        //$dt = $this->pubdate_list();
        $this->db->where('n_category', $n_category);
        $this->db->where('n_id !=', $n_id);
        $this->db->where('n_date', $dt);
