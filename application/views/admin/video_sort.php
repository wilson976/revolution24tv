<?php //print_r($videonews) ?>


<div class="row-fluid sortable">
    <div class="box span12">
        <div class="box-header well" data-original-title>
            <h2><i class="icon-edit"></i>Video List</h2>
            <div class="box-icon">                
        ...
        <div class="box-content">
            <?php echo form_open('admin/video_gallery/orderupdate_vedio/', 'class="form-horizontal"'); ?>            
            <fieldset> 

                <?php
                $i = 1;
                $row_count = count($videonews);
                // echo $row;
                if ($videonews != NULL) {
                    ?>
                    <table class="control-group">
                    <?php
                    foreach ($videonews as $vedio) {
                        ?><tr>
        
                        <td>
                            <label class="control-label" for="focusedInput" style="width: 400px; text-align: left;">
                            <?php
                            	echo $i . '.   ' . strip_tags($vedio['v_caption']); 
                            ?>
                            ...
                            <?php  if($i!=$row_count){ ?>
                                ...
                                <input name="v_id[]" type="hidden" value="<?php echo $vedio['v_id']; ?>">
                                <input name="v_sort[]" type="text" style="width: 100px;" class="input-xlarge focused sort" id="<?php echo 'v_sort' . $i ?>" value="<?php echo $vedio['v_sort']; ?>">
                            </div>                            
                            <?php

                            echo form_hidden('v_id' . $i, $vedio['v_id']);
                            //echo form_hidden('possition', $possition);
                            ?></td>
